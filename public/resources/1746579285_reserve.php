...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['reserve'])) {
    ...
    try {
        ...
        if ($insertQuery->execute()) {
            ...
            $updateSession = $conn->prepare("UPDATE users SET session = session - 1 WHERE idno = ?");
            $updateSession->bind_param("i", $idno);
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['delete_reservation'])) {
    ...
    try {
        ...
        $verifyQuery->bind_param("ii", $reservationId, $idno);
        $verifyQuery->execute();
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['sitin'])) {
    ...
    try {
        ...
        $reservationQuery = $conn->prepare("SELECT idno, lab_number, reservation_date, time_in, purpose FROM reservations WHERE id = ? AND idno = ? AND status = 'approved'");
        $reservationQuery->bind_param("ii", $reservationId, $idno);
        ...
        // Check if already sat in
        $checkQuery = $conn->prepare("SELECT id FROM sitin WHERE idno = ? AND sitin_date = ? AND time_in = ?");
        $checkQuery->bind_param("iss", $idno, $reservation['reservation_date'], $reservation['time_in']);
        $checkQuery->execute();
        $checkQuery->store_result();
        
        if ($checkQuery->num_rows > 0) {
            throw new Exception("You have already sat in for this reservation");
        }
        
        // Create sit-in record
        $insertQuery = $conn->prepare("INSERT INTO sitin (idno, lab_number, sitin_date, time_in, purpose) VALUES (?, ?, ?, ?, ?)");
        $insertQuery->bind_param("iisss", $idno, $reservation['lab_number'], $reservation['reservation_date'], $reservation['time_in'], $reservation['purpose']);
...
// Fetch user's reservations into $sitinData
$reservationsQuery = $conn->prepare("SELECT r.id, r.lab_number, r.reservation_date, r.time_in, r.purpose, r.status, 
                                    (SELECT COUNT(*) FROM sitin s WHERE s.idno = r.idno AND s.sitin_date = r.reservation_date AND s.time_in = r.time_in) as has_sitin
                                    FROM reservations r 
                                    WHERE r.idno = ? 
...
<body class="bg-gray-100 font-sans antialiased">
                                        ...
                                        <tr class="<?php echo ($index % 2 === 0) ? 'bg-gray-100' : 'bg-gray-200'; ?>">
                                            ...
                                            <td class="py-4 px-4 text-center">
                                                <?php if ($sitin['status'] == 'pending'): ?>
                                                    <button onclick="openEditModal(<?php echo $sitin['id']; ?>)" class="text-blue-500 hover:text-blue-700 mx-1">
                                                ...
                                                <?php elseif ($sitin['status'] == 'approved'): ?>
                                                    <?php if ($sitin['has_sitin'] > 0): ?>
                                                        <button class="bg-gray-300 text-gray-600 px-3 py-1 rounded text-sm cursor-not-allowed">
                                                            Sit-In
                                                        </button>
                                                    <?php else: ?>
                                                        <button onclick="recordSitin(<?php echo $sitin['id']; ?>)" class="bg-green-500 text-white px-3 py-1 rounded text-sm hover:bg-green-600">
                                                            Sit-In
                                                        </button>
                                                    <?php endif; ?>
                                                <?php else: ?>
    ...
    <div id="editModal" class="edit-modal">
        <div class="edit-modal-content">
            ...
            <form id="editReservationForm" class="space-y-4">
                ...
                <div class="flex flex-col text-left">
                    <label class="font-semibold">Lab Number:</label>
                    <select id="editLabNumber" name="lab_number" class="w-full border px-3 py-2 rounded bg-white" required>
                        <option value="524">524</option>
                        <option value="526">526</option>
                        <option value="528">528</option>
                        <option value="530">530</option>
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div class="flex flex-col text-left">
                        <label class="font-semibold">Date:</label>
                        <input type="text" id="editReservationDate" name="reservation_date" class="w-full border px-3 py-2 rounded bg-white" required>
                    </div>
                    
                    <div class="flex flex-col text-left">
                        <label class="font-semibold">Time:</label>
                        <input type="time" id="editTimeIn" name="time_in" class="w-full border px-3 py-2 rounded bg-white" required>
                    </div>
                </div>
                
                <div class="flex flex-col text-left">
                    <label class="font-semibold">Purpose:</label>
                    <select id="editPurpose" name="purpose" class="w-full border px-3 py-2 rounded bg-white" onchange="toggleEditOtherReason()" required>
                        <option value="C Programming">C Programming</option>
                        <option value="C# Programming">C# Programming</option>
                        <option value="Java Programming">Java Programming</option>
                        <option value="PHP Programming">PHP Programming</option>
                ...
                <div id="editOtherReasonDiv" class="hidden flex flex-col text-left">
                    <label class="font-semibold">Specify Purpose:</label>
                    <input type="text" id="editOtherReason" name="other_reason" class="w-full border px-3 py-2 rounded bg-white">
                </div>

                <div class="flex justify-center gap-4 pt-4">
                    <button type="button" onclick="closeEditModal()" class="w-40 h-12 border border-red-700 text-red-700 font-semibold rounded-lg hover:bg-red-700 hover:text-white transition duration-300">
                        Cancel
                    </button>
                    <button type="submit" class="w-40 h-12 bg-purple-700 text-white font-semibold rounded-lg hover:bg-purple-800 transition duration-300">
                        Update
    ...
    <script>
        ...
        const editDatePicker = flatpickr("#editReservationDate", {
            minDate: "today",
        ...
        function toggleEditOtherReason() {
            const purposeSelect = document.getElementById('editPurpose');
            const otherReasonDiv = document.getElementById('editOtherReasonDiv');
            
            if (purposeSelect.value === 'Others') {
                otherReasonDiv.classList.remove('hidden');
                document.getElementById('editOtherReason').required = true;
            } else {
                otherReasonDiv.classList.add('hidden');
                document.getElementById('editOtherReason').required = false;
            }
        }
        ...
        function deleteReservation(reservationId) {
            if (confirm('Are you sure you want to delete this reservation?')) {
                ...
                formData.append('delete_reservation', '1');
        ...
        // Record sit-in function
        function recordSitin(reservationId) {
            if (confirm('Are you sure you want to record your sit-in?')) {
                const formData = new FormData();
                formData.append('sitin', '1');
                formData.append('id', reservationId);
            ...
            fetch(`get_reservation.php?id=${reservationId}`)
                ...
                .then(data => {
                    if (data) {
                        ...
                        document.getElementById('editLabNumber').value = data.lab_number;
                        editDatePicker.setDate(data.reservation_date);
                        ...
                        if (data.purpose === 'Others') {
                            document.getElementById('editOtherReasonDiv').classList.remove('hidden');
                            document.getElementById('editOtherReason').value = data.purpose;
                            document.getElementById('editOtherReason').required = true;
                        } else {
                            document.getElementById('editOtherReasonDiv').classList.add('hidden');
                            document.getElementById('editOtherReason').required = false;
                        }
        ...
        document.getElementById('editReservationForm').addEventListener('submit', async function(e) {
            ...
            try {
                ...
                if (selectedDate) {
                    const formattedDate = editDatePicker.formatDate(selectedDate, 'Y-m-d');
                    formData.set('reservation_date', formattedDate);
                }
